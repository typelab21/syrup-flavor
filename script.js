...
` : ""}
  ...
  renderResultExtraContents(result);
...
/* ==================================================
   Result Extra Contents
   シロップおみくじ・手紙・シェア文メーカー
================================================== */

const syrupOmikujiMessages = [
  "今日は、無理に明るくしなくていい日。あなたの静かな甘さは、ちゃんと誰かの記憶に残っています。",
  "今日は、少しだけ自分に甘くしていい日。頑張るより、まずは心をほどく一滴を。",
  "今日は、言葉にすることで愛されやすくなる日。察してほしい気持ちを、少しだけ外に出してみて。",
  "今日は、距離感を大切にしたい日。近づくことだけが愛じゃなく、心地よくいられる場所を選んでいい。",
  "今日は、あなたの余韻が強く残る日。その場で伝わらなくても、あとから思い出される甘さがあります。",
  "今日は、誰かに合わせすぎない日。あなたの味を薄めなくても、ちゃんと好きになってくれる人はいます。",
  "今日は、やさしさを配りすぎない日。大切に味わってくれる人にだけ、甘さを注いでいい。",
  "今日は、少し素直になると恋が進む日。照れ隠しの奥にある本音を、ひとさじだけ見せてみて。",
  "今日は、無理に完璧でいなくていい日。隙や揺れも、あなたを愛しく見せる大切な香りです。",
  "今日は、待つより選ぶ日。あなたを雑に扱わない場所へ、自分から心を向けていい。",
  "今日は、軽やかさの中に本音を混ぜたい日。笑って流す前に、ほんの少しだけ気持ちを残して。",
  "今日は、静かに整える日。焦って答えを出さなくても、あなたの甘さはゆっくり深まります。"
];

function getRandomOmikujiMessage() {
  const index = Math.floor(Math.random() * syrupOmikujiMessages.length);
  return syrupOmikujiMessages[index];
}

function buildShareText(result) {
  const detail = typeof resultDetails !== "undefined" ? resultDetails[result.name] : null;
  const title = detail?.loveStyleTitle || result.catch || "自分らしい甘さで愛されるタイプ";

  return `私は【${result.name}】でした🍯
...
function renderResultExtraContents(result) {
  ...
  if (omikujiText) {
    omikujiText.textContent = "今日のあなたに必要な一滴を引いてみてください。";
  }

  if (omikujiButton && omikujiText) {
    omikujiButton.onclick = () => {
      omikujiText.textContent = getRandomOmikujiMessage();
    };
  }

  if (shareTextBox) {
    shareTextBox.value = buildShareText(result);
  }

  if (copyShareTextButton && shareTextBox) {
    copyShareTextButton.onclick = () => {
      copyTextToClipboard(shareTextBox.value)
        .then(() => {
          if (copyShareStatus) {
            copyShareStatus.textContent = "コピーしました。SNSに貼り付けて使えます。";
          }
        })
        .catch(() => {
          if (copyShareStatus) {
            copyShareStatus.textContent = "コピーできませんでした。文章を選択して手動でコピーしてください。";
          }
        });
    };
  }
...
const rankingBottleColors = {
  "キャラメルシロップ": ["#c88a3d", "#f4c271"],
  "アールグレイシロップ": ["#8d7b95", "#c9bdd5"],
  "ラムレーズンシロップ": ["#6f4a7c", "#b48ac6"],
  "メープルシロップ": ["#b87932", "#f0b45d"],

  "ミルクシロップ": ["#eadfd4", "#fff8f0"],
  "ハニーバターシロップ": ["#e4a83d", "#ffe08a"],
  "カフェオレシロップ": ["#a97856", "#dfc0a4"],
  "ホワイトチョコシロップ": ["#e8d7c7", "#fff7ee"],

  "ストロベリーシロップ": ["#e85b8d", "#ff9fbd"],
  "マシュマロシロップ": ["#e6a8c9", "#ffd9ec"],
  "ピーチシロップ": ["#f39a7d", "#ffc4ad"],
  "ミルクティーシロップ": ["#b98a63", "#e8c7a5"],

  "ダークチョコシロップ": ["#4b2b2f", "#8a5a5f"],
  "シナモンシロップ": ["#b85f3a", "#e99a6d"],
  "ピスタチオシロップ": ["#7fa76a", "#c2d99b"],
  "レモンシロップ": ["#f1cf3d", "#fff08a"]
};

const flavorRankings = [
  {
    id: "lonely-hidden",
    title: "さみしさを隠しがちなフレーバーランキング",
    subtitle:
      "平気なふりをする・相手に負担をかけたくない・寂しいと言う前に自分で処理しがちな度",
    items: [
      {
        name: "アールグレイシロップ",
        text: "寂しくても、すぐには言葉にしないタイプ。自分の世界を守りながら、静かに気持ちを整理しがち。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "本音を簡単には見せないタイプ。寂しさにも奥行きがあり、気づかれる前に隠してしまう。"
      },
      {
        name: "キャラメルシロップ",
        text: "相手に迷惑をかけたくなくて、寂しさを飲み込みがち。あとから静かに距離を置くことも。"
      },
      {
        name: "ミルクティーシロップ",
        text: "落ち着いて見えるぶん、寂しさに気づかれにくい。穏やかに我慢してしまう。"
      },
      {
        name: "カフェオレシロップ",
        text: "軽く笑って流すのが上手。本当は寂しくても、冗談っぽくごまかしがち。"
      },
      {
        name: "ピスタチオシロップ",
        text: "寂しい時ほど一人の領域に戻るタイプ。わかってほしいけど、踏み込まれすぎるのも苦手。"
      },
      {
        name: "メープルシロップ",
        text: "頼られる側に回りやすく、自分の寂しさは後回し。静かに疲れをためやすい。"
      },
      {
        name: "ダークチョコシロップ",
        text: "寂しさを認めるより、相手の本気度を見ようとするタイプ。強く見えて内側は深い。"
      },
      {
        name: "ミルクシロップ",
        text: "相手の気持ちを優先しすぎて、自分の寂しさを小さく扱いがち。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "傷ついてもやわらかく隠すタイプ。寂しいと言うまでに少し時間がかかる。"
      },
      {
        name: "ハニーバターシロップ",
        text: "明るく振る舞うことで寂しさを隠しがち。元気な自分を求められると疲れる。"
      },
      {
        name: "マシュマロシロップ",
        text: "寂しさは出るけれど、強くは言えないタイプ。やさしく気づいてほしい。"
      },
      {
        name: "ピーチシロップ",
        text: "寂しさは比較的出やすいけれど、冗談にしてしまうこともある。"
      },
      {
        name: "ストロベリーシロップ",
        text: "寂しさは隠すより、態度や表情に出やすいタイプ。気づいてほしい気持ちが素直。"
      },
      {
        name: "レモンシロップ",
        text: "明るく切り替えようとするけれど、本音は見え隠れする。軽やかに寂しさを逃がす。"
      },
      {
        name: "シナモンシロップ",
        text: "寂しさを隠すというより、強い言葉や刺激に変換しがち。素直になるまでが勝負。"
      }
    ]
  },
  ...
  {
    id: "addictive",
    title: "沼らせ力が強いフレーバーランキング",
    subtitle:
      "忘れにくい・もっと知りたくなる・気づいたら考えてしまう余韻の強さランキング",
    items: [
      {
        name: "ダークチョコシロップ",
        text: "甘さだけで終わらない濃さが強い。深く関わった相手ほど、簡単には忘れられない。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "見えない部分が多く、知るほど気になるタイプ。余白と秘密が沼になる。"
      },
      {
        name: "シナモンシロップ",
        text: "甘さに刺激が混ざるタイプ。予想できない反応が、相手の心を何度も揺らす。"
      },
      {
        name: "ピスタチオシロップ",
        text: "わかる人には深く刺さる個性派。代わりがきかない特別感が強い。"
      },
      {
        name: "アールグレイシロップ",
        text: "静かな香りのように残るタイプ。近づきすぎない余白が、相手を引き寄せる。"
      },
      {
        name: "キャラメルシロップ",
        text: "その場より、あとから効くタイプ。離れた時に存在の大きさを思い出される。"
      },
      {
        name: "レモンシロップ",
        text: "爽やかな刺激で記憶に残る。明るさの奥にある本音がクセになる。"
      },
      {
        name: "ストロベリーシロップ",
        text: "目が離せない甘さがあるタイプ。感情の揺れが相手の庇護欲を刺激する。"
      },
      {
        name: "カフェオレシロップ",
        text: "気楽なのに深い話までできる。友達感覚から抜け出せなくなる魅力。"
      },
      {
        name: "ハニーバターシロップ",
        text: "一緒にいると前向きになれる。疲れた時にふと思い出される甘さ。"
      },
      {
        name: "ミルクティーシロップ",
        text: "毎日飲みたくなる安心感。刺激よりも、生活に馴染むことで離れがたくなる。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "傷ついた心を包む甘さ。弱い部分を見せた相手には、かなり特別に残る。"
      },
      {
        name: "メープルシロップ",
        text: "帰りたくなるぬくもりが強い。派手ではないけれど、長く残る安心感。"
      },
      {
        name: "ピーチシロップ",
        text: "自然な近さで日常に入り込む。気づいたら大切になっているタイプ。"
      },
      {
        name: "マシュマロシロップ",
        text: "守りたくなるやわらかさ。刺激よりも、癒しとして心に残る。"
      },
      {
        name: "ミルクシロップ",
        text: "まろやかな安心感で惹きつける。沼らせというより、戻りたくなる存在。"
      }
    ]
  },
  ...
  {
    id: "sweet-clumsy",
    title: "実は甘え下手なフレーバーランキング",
    subtitle:
      "本当は甘えたいのに、遠慮する・強がる・自分の気持ちを後回しにしがちな度",
    items: [
      {
        name: "ダークチョコシロップ",
        text: "甘えたい気持ちを見せる前に、相手を信じられるか見てしまう。弱さを出すまでが長い。"
      },
      {
        name: "アールグレイシロップ",
        text: "甘えるより、距離感を整えがち。踏み込まれるのは苦手だけど、丁寧に近づかれたい。"
      },
      {
        name: "ピスタチオシロップ",
        text: "自由でいたい気持ちが強く、甘え方が独特。自分の世界ごと受け止められると弱い。"
      },
      {
        name: "キャラメルシロップ",
        text: "迷惑をかけたくなくて、甘えたい気持ちを後回しにしがち。察してくれる人に弱い。"
      },
      {
        name: "ミルクティーシロップ",
        text: "落ち着いて見られやすく、甘えたい時も平気なふりをしがち。"
      },
      {
        name: "メープルシロップ",
        text: "支える側になりやすく、自分が頼るのは少し苦手。安心できる相手にはゆっくり甘える。"
      },
      {
        name: "カフェオレシロップ",
        text: "本音を冗談っぽく混ぜるタイプ。真剣に甘えるのは少し照れくさい。"
      },
      {
        name: "シナモンシロップ",
        text: "甘えたいのに、強い言葉やからかいに変換しがち。素直になった時の破壊力が高い。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "深く信頼しないと甘えないタイプ。開いた相手にはかなり濃くなる。"
      },
      {
        name: "レモンシロップ",
        text:          "明るく軽やかに見せるぶん、真面目に甘えるのが少し照れくさいタイプ。本音は笑顔の奥に隠れがち。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "甘えたい気持ちはあるけれど、相手を困らせたくなくて遠慮しがち。やさしく受け止められると安心する。"
      },
      {
        name: "ミルクシロップ",
        text: "相手に合わせるのが上手すぎて、自分の甘えたい気持ちを後回しにしやすい。"
      },
      {
        name: "ハニーバターシロップ",
        text: "明るく支える側に回りやすいタイプ。弱っている自分を見せるのは少し苦手。"
      },
      {
        name: "マシュマロシロップ",
        text: "甘える力はあるけれど、嫌われたくなくて控えめになりがち。安心すると素直にほどける。"
      },
      {
        name: "ピーチシロップ",
        text: "自然に甘えられるけれど、本気の寂しさは冗談にしがち。軽く見せてしまうことも。"
      },
      {
        name: "ストロベリーシロップ",
        text: "甘えたい気持ちが比較的まっすぐ出やすいタイプ。好きな人には素直に気にかけてほしい。"
      }
    ]
  },
    {
    id: "easy-to-read-love",
    title: "好きバレしやすいフレーバーランキング",
    subtitle:
      "好きな人への反応・視線・態度・テンションの変化が周りに伝わりやすい度",
    items: [
      {
        name: "ストロベリーシロップ",
        text: "好きな人への反応がかなり素直。嬉しさも寂しさも表情に出やすく、周りにも気づかれやすい。"
      },
      {
        name: "ピーチシロップ",
        text: "自然に距離を縮めるのが上手。好きな人には話しかける回数や反応の甘さが増えやすい。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "相手を大切にしたい気持ちが態度に出るタイプ。好きな人には言葉選びがさらにやさしくなる。"
      },
      {
        name: "ハニーバターシロップ",
        text: "好きな人を喜ばせたくて、つい明るく支えに行く。テンションの変化でバレやすい。"
      },
      {
        name: "ミルクシロップ",
        text: "好きな人には合わせ方がさらに丁寧になる。本人は自然のつもりでも、周りから見るとわかりやすい。"
      },
      {
        name: "レモンシロップ",
        text: "好きな人には絡み方が増えやすい。軽い冗談や反応の速さで、好意がにじみやすい。"
      },
      {
        name: "シナモンシロップ",
        text: "素直な好意より、からかいや強めの反応に出がち。逆に意識しているのがバレるタイプ。"
      },
      {
        name: "マシュマロシロップ",
        text: "好きな人の前ではやわらかさが増える。控えめだけど、安心したい気持ちが態度に出やすい。"
      },
      {
        name: "カフェオレシロップ",
        text: "会話の温度で好意が出るタイプ。好きな人には軽い話の中に少しだけ本音を混ぜがち。"
      },
      {
        name: "メープルシロップ",
        text: "好きな人には支えたい気持ちが出る。派手ではないけれど、気遣いの回数でバレる。"
      },
      {
        name: "ミルクティーシロップ",
        text: "落ち着いているけれど、本命には態度が少しやわらかくなる。近くで見る人にはわかる。"
      },
      {
        name: "キャラメルシロップ",
        text: "好意は静かに出るタイプ。周りより本人にだけ伝わる小さな気遣いが増える。"
      },
      {
        name: "ダークチョコシロップ",
        text: "好きな人ほど慎重に見るので、表には出にくい。ただし視線や態度の重みで気づかれることも。"
      },
      {
        name: "ピスタチオシロップ",
        text: "好意を大きく見せるより、自分の世界に少し招くタイプ。わかる人にはわかる好きバレ。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "好きでもすぐには見せないタイプ。会話の深さや本音の量で、じわっとバレる。"
      },
      {
        name: "アールグレイシロップ",
        text: "好意をかなり丁寧に隠すタイプ。好きバレしにくいけれど、言葉の選び方には少し出る。"
      }
    ]
  },
  ...
  {
    id: "love-complicated",
    title: "恋愛で拗らせやすいフレーバーランキング",
    subtitle:
      "好きなのに素直になれない・考えすぎる・相手の反応を深読みしやすい度",
    items: [
      {
        name: "ダークチョコシロップ",
        text: "本気だからこそ深読みしやすい。軽く扱われることに敏感で、信頼までに時間がかかる。"
      },
      {
        name: "シナモンシロップ",
        text: "好きなのに素直になれず、強い言葉やからかいに変換しがち。照れ隠しが拗れやすい。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "本音を見せたいのに、簡単には開けないタイプ。深く知ってほしい気持ちが複雑に出る。"
      },
      {
        name: "アールグレイシロップ",
        text: "距離感を考えすぎて、近づくタイミングを逃しがち。雑に踏み込まれると一気に閉じる。"
      },
      {
        name: "ピスタチオシロップ",
        text: "自由でいたい気持ちと、特別に理解されたい気持ちがぶつかりやすい。"
      },
      {
        name: "キャラメルシロップ",
        text: "察してほしい気持ちを抱えがち。優しさを当たり前にされると静かに傷つく。"
      },
      {
        name: "カフェオレシロップ",
        text: "軽く見せるのが上手すぎて、本音が伝わりにくい。内側ではかなり考えている。"
      },
      {
        name: "ミルクティーシロップ",
        text: "落ち着いて見えるぶん、不安を言い出しにくい。静かに我慢して拗れやすい。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "傷つけたくなくて言えないことが増えがち。やさしさと我慢の境目で揺れる。"
      },
      {
        name: "ストロベリーシロップ",
        text: "不安が態度に出やすいタイプ。安心が足りないと、試すような反応になりがち。"
      },
      {
        name: "ミルクシロップ",
        text: "相手に合わせすぎて、自分の本音が後回しになりやすい。気づくと苦しくなっている。"
      },
      {
        name: "マシュマロシロップ",
        text: "嫌なことを強く言えず、心の中にためやすい。やさしく気づいてほしいタイプ。"
      },
      {
        name: "メープルシロップ",
        text: "支えすぎて自分が疲れることがある。安定を求めるぶん、急な変化に弱い。"
      },
      {
        name: "レモンシロップ",
        text: "本音を明るさで逃がしがち。深刻に見せないことで、逆に伝わりにくくなる。"
      },
      {
        name: "ハニーバターシロップ",
        text: "明るく支えようと頑張りすぎるタイプ。弱っている自分を見せるのが少し苦手。"
      },
      {
        name: "ピーチシロップ",
        text: "比較的自然に距離を縮められるタイプ。ただし曖昧な関係が続くと不安になりやすい。"
      }
    ]
  },
    {
    id: "chase-more-than-be-chased",
    title: "追われるより追いたいフレーバーランキング",
    subtitle:
      "自分から気になっていく・簡単に手に入らない相手に惹かれる・恋に少し刺激を求めやすい度",
    items: [
      {
        name: "シナモンシロップ",
        text: "簡単に安心しきるより、少し刺激がある相手に惹かれやすい。追うほど恋の温度が上がるタイプ。"
      },
      {
        name: "レモンシロップ",
        text: "テンポよく心が動く恋に弱い。相手の反応を引き出したくなり、自分から近づきやすい。"
      },
      {
        name: "ダークチョコシロップ",
        text: "軽く追うというより、深く知りたくなる相手を追うタイプ。簡単に見えない相手に惹かれる。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "相手の奥行きが見えるほど気になる。全部が見えない関係に、静かに引き込まれやすい。"
      },
      {
        name: "ピスタチオシロップ",
        text: "自分の感性に刺さる相手だと、かなり気になり続ける。普通じゃない魅力に弱い。"
      },
      {
        name: "アールグレイシロップ",
        text: "誰にでも開かない相手に惹かれやすい。丁寧に距離を縮める過程そのものを大切にする。"
      },
      {
        name: "カフェオレシロップ",
        text: "会話のテンポが合う相手には自分から近づきやすい。軽さの奥に深さがある人に惹かれる。"
      },
      {
        name: "ストロベリーシロップ",
        text: "追いたい気持ちもあるけれど、安心させてほしい気持ちも強い。反応がある相手に弱い。"
      },
      {
        name: "ピーチシロップ",
        text: "自然に距離を縮めるのが上手。追うというより、気づいたら近くにいるタイプ。"
      },
      {
        name: "ハニーバターシロップ",
        text: "気になる相手を明るく元気づけたくなる。追う時もあたたかく、重くなりすぎない。"
      },
      {
        name: "メープルシロップ",
        text: "刺激的に追うより、相手を支えながら距離を縮めるタイプ。安定が見える恋に向いている。"
      },
      {
        name: "ミルクティーシロップ",
        text: "急に追いかけるより、穏やかに近づくタイプ。安心できる相手にはゆっくり心を向ける。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "追うより、相手から丁寧に大切にされると心を開きやすい。強引な恋は少し苦手。"
      },
      {
        name: "マシュマロシロップ",
        text: "自分から追うより、安心できる相手に受け止めてもらいたいタイプ。やさしい接近に弱い。"
      },
      {
        name: "ミルクシロップ",
        text: "追うより合わせる恋になりやすい。相手が近づきやすい空気を作るのが得意。"
      },
      {
        name: "キャラメルシロップ",
        text: "自分から強く追うより、静かに想いを温めるタイプ。焦らず距離を縮めたい。"
      }
    ]
  },

  {
    id: "after-breakup-memory",
    title: "別れたあと引きずらせるフレーバーランキング",
    subtitle:
      "離れてから存在の大きさに気づかれる・日常の中で思い出される・余韻が長く残りやすい度",
    items: [
      {
        name: "キャラメルシロップ",
        text: "別れたあとに一番効いてくるタイプ。何気ない優しさや安心感が、日常の中でじわじわ思い出される。"
      },
      {
        name: "ダークチョコシロップ",
        text: "深く向き合った記憶が強く残る。甘さだけではない濃さが、別れたあとも忘れにくい。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "全部を知りきれなかった余白が残るタイプ。離れてから、もっと知りたかったと思わせる。"
      },
      {
        name: "ミルクティーシロップ",
        text: "穏やかな日常の記憶が残りやすい。疲れた時に、ふと戻りたくなる安心感がある。"
      },
      {
        name: "アールグレイシロップ",
        text: "静かな香りのように記憶に残る。言葉や雰囲気が、あとから何度も思い出される。"
      },
      {
        name: "メープルシロップ",
        text: "支えてくれた記憶が残るタイプ。失ってから、どれだけ安心させてもらっていたか気づかれる。"
      },
      {
        name: "カフェオレシロップ",
        text: "何でもない会話の心地よさがあとから効く。気楽に話せる相手だったと気づかれやすい。"
      },
      {
        name: "ピスタチオシロップ",
        text: "代わりがきかない個性が残る。離れてから、あの独特さは他にないと思わせるタイプ。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "やさしく受け止めてくれた記憶が残る。弱い部分を見せた相手ほど、あとから思い出す。"
      },
      {
        name: "ハニーバターシロップ",
        text: "一緒にいると前向きになれた記憶が残る。落ち込んだ時に、明るさとぬくもりを思い出される。"
      },
      {
        name: "ストロベリーシロップ",
        text: "表情や感情の動きが記憶に残る。守りたかった気持ちを、あとから思い出させる。"
      },
      {
        name: "シナモンシロップ",
        text: "刺激と甘さの両方が残るタイプ。楽しかった衝突や強い反応まで、印象として残りやすい。"
      },
      {
        name: "レモンシロップ",
        text: "明るさやテンポの良さが残る。日常が少し退屈になった時に、ふと思い出される。"
      },
      {
        name: "ピーチシロップ",
        text: "自然に近くにいた記憶が残る。何気ないやりとりほど、あとから懐かしくなる。"
      },
      {
        name: "マシュマロシロップ",
        text: "やわらかく受け止めてくれた記憶が残る。強い余韻というより、やさしい懐かしさになる。"
      },
      {
        name: "ミルクシロップ",
        text: "安心して話せた記憶が残る。派手ではないけれど、心が疲れた時に戻りたくなる。"
      }
    ]
  },
  ...
  {
    id: "too-loyal",
    title: "実は一途すぎるフレーバーランキング",
    subtitle:
      "好きになった相手を長く大切にする・簡単に気持ちが薄れない・情が深く残りやすい度",
    items: [
      {
        name: "ダークチョコシロップ",
        text: "一度本気になるとかなり深い。軽く好きになるより、覚悟を持って相手を見続けるタイプ。"
      },
      {
        name: "メープルシロップ",
        text: "長く支え続ける一途さがある。派手ではないけれど、日常ごと大切にする愛情が強い。"
      },
      {
        name: "キャラメルシロップ",
        text: "静かに想いを温め続けるタイプ。表に出しすぎなくても、気持ちはかなり長く残る。"
      },
      {
        name: "ミルクティーシロップ",
        text: "穏やかに長く愛するタイプ。信頼した相手には、安定した愛情を注ぎ続ける。"
      },
      {
        name: "ラムレーズンシロップ",
        text: "心を開くまで慎重なぶん、開いた相手にはかなり深い。軽く切り替えるのは苦手。"
      },
      {
        name: "ホワイトチョコシロップ",
        text: "大切にしたい気持ちが長く残るタイプ。相手を傷つけないように考え続ける一途さがある。"
      },
      {
        name: "ミルクシロップ",
        text: "相手を受け止め続ける愛情が深い。好きな人のために自分を調整しすぎることも。"
      },
      {
        name: "マシュマロシロップ",
        text: "安心した相手にはやわらかく一途。強く言えないぶん、心の中で長く大切にする。"
      },
      {
        name: "アールグレイシロップ",
        text: "簡単には心を開かないけれど、選んだ相手への気持ちは静かに長く続きやすい。"
      },
      {
        name: "ストロベリーシロップ",
        text: "好きな人への気持ちはまっすぐ。安心できる相手には、かなり強く心を向ける。"
      },
      {
        name: "ハニーバターシロップ",
        text: "好きな人を元気にしたい気持ちが続くタイプ。明るく支える一途さがある。"
      },
      {
        name: "カフェオレシロップ",
        text: "重く見せないけれど、内側ではちゃんと大切にしている。気楽さの奥に一途さがある。"
      },
      {
        name: "ピーチシロップ",
        text: "自然体で大切にし続けるタイプ。身近な相手ほど、長く甘さを向ける。"
      },
      {
        name: "ピスタチオシロップ",
        text: "自分の世界に入れた相手には特別。一途だけど、自由な距離感も大切にする。"
      },
      {
        name: "シナモンシロップ",
        text: "熱量は高いけれど、刺激やテンポも大事。相手によって一途さが強く出るタイプ。"
      },
      {
        name: "レモンシロップ",
        text: "一途でも軽やかに見せるタイプ。気持ちを重く見せないぶん、切り替えが早く見られがち。"
      }
    ]
  }
];

function getRankingType(name) {
  return baseTypes.find((type) => type.name === name);
}

function createRankingBottle(name) {
  const colors = rankingBottleColors[name] || ["#ec91c6", "#ad85e8"];
  const shortName = name.replace("シロップ", "");

  return `
    <div class="ranking-bottle" style="--rank-main:${colors[0]}; --rank-light:${colors[1]};">
      <div class="ranking-bottle-label">${shortName}</div>
    </div>
  `;
}
...
function renderRankingTabs() {
  ...
  flavorRankings.forEach((ranking, index) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "ranking-tab";
    if (index === 0) button.classList.add("is-active");

    button.textContent = ranking.title.replace("フレーバーランキング", "");
    ...
    button.addEventListener("click", () => {
      ...
      renderRankingBoard(ranking.id);
    });

    rankingTabs.appendChild(button);
  });
}

function renderRankingBoard(rankingId = flavorRankings[0].id) {
  const rankingBoard = document.getElementById("rankingBoard");
  if (!rankingBoard) return;

  const ranking = flavorRankings.find((item) => item.id === rankingId) || flavorRankings[0];
  ...
  rankingBoard.innerHTML = `
    <div class="ranking-title-area">
      <p class="ranking-label">SYRUP TYPES</p>
      <h3>${ranking.title}</h3>
      <p>${ranking.subtitle}</p>
    </div>

    <div class="ranking-top3">
      ${top3
        .map((item, index) => {
          const type = getRankingType(item.name);

          return `
            <button type="button" class="ranking-hero-card" data-type-name="${item.name}">
              <div class="ranking-no">
                <span>No.</span>${index + 1}
              </div>

              ${createRankingBottle(item.name)}
    ...
    <div class="ranking-grid">
      ${rest
        .map((item, index) => {
          ...
          return `
            <button type="button" class="ranking-mini-card" data-type-name="${item.name}">
              <div class="ranking-mini-no">No.${rank}</div>
  ...
  rankingBoard.querySelectorAll("[data-type-name]").forEach((card) => {
    card.addEventListener("click", () => {
      const typeName = card.dataset.typeName;
      const type = getRankingType(typeName);
...
initFlavorRanking();
/* ==================================================
   Ranking Animation Refresh
   タブ切り替えのたびにランキングをふわっと表示
================================================== */

function refreshRankingAnimation() {
  const rankingBoard = document.getElementById("rankingBoard");
  if (!rankingBoard) return;

  rankingBoard.classList.remove("is-ranking-animated");

  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      rankingBoard.classList.add("is-ranking-animated");
    });
  });
}

const originalRenderRankingBoard = renderRankingBoard;

renderRankingBoard = function (rankingId) {
  originalRenderRankingBoard(rankingId);
  refreshRankingAnimation();
};

refreshRankingAnimation();
/* ==================================================
   Ranking Extra Animation Effects
================================================== */

function addHeroShineToRankingCards() {
  const heroCards = document.querySelectorAll(".ranking-hero-card");

  heroCards.forEach((card) => {
    if (!card.querySelector(".hero-shine")) {
...
function sparkleRankingBoard() {
  const rankingBoard = document.getElementById("rankingBoard");
  if (!rankingBoard) return;

  rankingBoard.classList.remove("ranking-spark");

  requestAnimationFrame(() => {
    rankingBoard.classList.add("ranking-spark");
...
const previousRenderRankingBoardForEffects = renderRankingBoard;

renderRankingBoard = function (rankingId) {
  previousRenderRankingBoardForEffects(rankingId);
  addHeroShineToRankingCards();
  sparkleRankingBoard();
};
...
function createRankingBurst(x, y) {
  const burst = document.createElement("div");
  burst.className = "ranking-burst";
  ...
  positions.forEach(([moveX, moveY], index) => {
    const particle = document.createElement("span");
    particle.style.setProperty("--move-x", `${moveX}px`);
    particle.style.setProperty("--move-y", `${moveY}px`);
    particle.style.animation = "none";
    particle.offsetHeight;
    particle.style.animation = `rankingBurstParticleSafe 0.72s ease-out forwards`;
...
document.addEventListener("click", (event) => {
  const target = event.target.closest(".ranking-tab, .ranking-hero-card, .ranking-mini-card");
  if (!target) return;

  createRankingBurst(event.clientX, event.clientY);
});
/* ==================================================
   Ranking Editorial Decorations
   タグ・リボン・型ラベル・メモを後付けする
================================================== */

const rankingEditorialData = {
  "lonely-hidden": {
    tags: ["平気なふり", "寂しさ耐性", "本音を隠す", "恋愛のクセ"],
    ribbons: ["孤高の我慢", "秘密の余白", "静かな寂しさ"],
    memo:
      "このランキングは、寂しさを感じた時にどれだけ表に出さず、自分の中で処理しようとするかを基準にしています。上位ほど「平気」と言いながら、内側で静かに揺れやすいタイプです。"
  },
  "addictive": {
    tags: ["沼らせ", "余韻", "忘れにくい", "中毒性"],
    ribbons: ["濃厚余韻", "秘密の中毒性", "刺激のスパイス"],
    memo:
      "このランキングは、離れたあとにも思い出されやすい余韻や、もっと知りたいと思わせる奥行きを基準にしています。上位ほど、甘さだけでは終わらない記憶の残し方をします。"
  },
  "sweet-clumsy": {
    tags: ["甘え下手", "遠慮", "強がり", "本音待ち"],
    ribbons: ["弱さガード", "上品な遠慮", "自由な甘え下手"],
    memo:
      "このランキングは、本当は甘えたいのに遠慮したり、強がったり、自分の気持ちを後回しにしやすいかを基準にしています。上位ほど、甘えたい気持ちを見せるまでに時間がかかります。"
  },
  "heavy-when-serious": {
    tags: ["本命", "重め愛", "深読み", "本気度"],
    ribbons: ["濃すぎる本命", "安心欲しがり", "照れ隠し高温"],
    memo:
      "このランキングは、好きが本気になった時にどれだけ深く考え、相手の反応に敏感になりやすいかを基準にしています。上位ほど、軽い恋より濃い関係を求めます。"
  },
  "easy-to-read-love": {
    tags: ["好きバレ", "態度に出る", "視線", "反応"],
    ribbons: ["素直な甘さ", "自然な接近", "やさしさ漏れ"],
    memo:
      "このランキングは、好きな人への反応や態度の変化がどれだけ周りに伝わりやすいかを基準にしています。上位ほど、本人は隠しているつもりでも好意がにじみます。"
  },
  "love-complicated": {
    tags: ["拗らせ", "深読み", "素直になれない", "恋愛癖"],
    ribbons: ["深読み沼", "照れ隠し事故", "本音の迷宮"],
    memo:
      "このランキングは、好きなのに素直になれなかったり、相手の反応を深読みしやすいかを基準にしています。上位ほど、恋が進むほど頭の中が忙しくなりやすいタイプです。"
  },
  "chase-more-than-be-chased": {
    tags: ["追いたい", "刺激", "気になる相手", "恋の温度"],
    ribbons: ["刺激ハンター", "軽やか追跡", "深掘り衝動"],
    memo:
      "このランキングは、自分から気になっていく恋や、簡単に手に入らない相手に惹かれやすいかを基準にしています。上位ほど、恋に少しスパイスがあるほど燃えやすいです。"
  },
  "after-breakup-memory": {
    tags: ["余韻", "引きずらせ", "忘れられない", "日常に残る"],
    ribbons: ["あとから効く", "濃厚記憶", "未完の余白"],
    memo:
      "このランキングは、離れたあとにどれだけ存在の大きさを思い出されやすいかを基準にしています。上位ほど、別れたあと日常の中でふと蘇るタイプです。"
  },
  "too-loyal": {
    tags: ["一途", "情が深い", "長く残る", "誠実"],
    ribbons: ["深愛タイプ", "日常ごと愛す", "静かな一途"],
    memo:
      "このランキングは、好きになった相手を長く大切にする力や、簡単に気持ちが薄れない情の深さを基準にしています。上位ほど、軽く切り替えるより長く想いを持ち続けます。"
  },
  "lover-only-gap": {
    tags: ["恋人限定", "ギャップ", "特別扱い", "心を許す"],
    ribbons: ["濃厚ギャップ", "上品な甘さ", "特別枠"],
    memo:
      "このランキングは、普段の印象と恋人にだけ見せる態度の差を基準にしています。上位ほど、心を許した相手にだけ見せる甘さや弱さが強く出ます。"
  }
};

function getRankingGroupInfo(typeName) {
  const type = typeof baseTypes !== "undefined"
    ? baseTypes.find((item) => item.name === typeName)
    : null;

  const group = type?.group || "";

  if (group === "余韻型") {
    return { label: "余韻型", className: "group-yoin" };
  }

  if (group === "共鳴型") {
    return { label: "共鳴型", className: "group-kyomei" };
  }

  if (group === "庇護型") {
    return { label: "庇護型", className: "group-higo" };
  }

  if (group === "執着型") {
    return { label: "執着型", className: "group-shuchaku" };
  }

  return { label: "Syrup", className: "" };
}

function getCurrentRankingDataFromBoard() {
  const title = document.querySelector(".ranking-title-area h3")?.textContent?.trim();
  if (!title || typeof flavorRankings === "undefined") return null;

  return flavorRankings.find((ranking) => ranking.title === title) || null;
}

function enhanceRankingEditorialDesign() {
  const rankingBoard = document.getElementById("rankingBoard");
  if (!rankingBoard) return;

  const ranking = getCurrentRankingDataFromBoard();
  if (!ranking) return;

  const editorial = rankingEditorialData[ranking.id] || {
    tags: ["シロップ研究", "恋愛傾向", "16タイプ", "ランキング"],
    ribbons: ["注目No.1", "注目No.2", "注目No.3"],
    memo:
      "このランキングは、各シロップの愛され方や恋愛で出やすい傾向をもとに並べています。自分のタイプだけでなく、気になる相手のタイプも眺めて楽しめます。"
  };

  const titleArea = rankingBoard.querySelector(".ranking-title-area");

  if (titleArea && !titleArea.querySelector(".ranking-theme-tags")) {
    const tags = document.createElement("div");
    tags.className = "ranking-theme-tags";
    tags.innerHTML = editorial.tags.map((tag) => `<span>${tag}</span>`).join("");
    titleArea.appendChild(tags);
  }

  rankingBoard.querySelectorAll(".ranking-hero-card").forEach((card, index) => {
    const typeName = card.dataset.typeName;
    const group = getRankingGroupInfo(typeName);
    const textArea = card.querySelector(".ranking-hero-text");

    if (textArea && !textArea.querySelector(".ranking-ribbon")) {
      const ribbon = document.createElement("div");
      ribbon.className = "ranking-ribbon";
      ribbon.textContent = editorial.ribbons[index] || `No.${index + 1} Pick`;
      textArea.prepend(ribbon);
    }

    if (textArea && !textArea.querySelector(".ranking-type-group")) {
      const groupLabel = document.createElement("div");
      groupLabel.className = `ranking-type-group ${group.className}`;
      groupLabel.textContent = group.label;
      const heading = textArea.querySelector("h4");
      if (heading) {
        textArea.insertBefore(groupLabel, heading);
      }
    }
  });

  rankingBoard.querySelectorAll(".ranking-mini-card").forEach((card) => {
    if (card.querySelector(".ranking-type-group")) return;
    ...
    if (heading) {
      ...
      groupLabel.className = `ranking-type-group ${group.className}`;
      groupLabel.textContent = group.label;
      card.insertBefore(groupLabel, heading);
    }
  });

  if (!rankingBoard.querySelector(".ranking-memo")) {
    const memo = document.createElement("div");
    memo.className = "ranking-memo";
    memo.innerHTML = `
      <strong>Ranking Memo</strong>
      ${editorial.memo}
    `;
    rankingBoard.appendChild(memo);
  }

  if (!rankingBoard.querySelector(".ranking-note-watermark")) {
    const watermark = document.createElement("div");
    watermark.className = "ranking-note-watermark";
    watermark.textContent = "Love Type Lab / Syrup Ranking Note";
    rankingBoard.appendChild(watermark);
  }
}

const previousRenderRankingBoardForEditorial = renderRankingBoard;

renderRankingBoard = function (rankingId) {
  previousRenderRankingBoardForEditorial(rankingId);
  enhanceRankingEditorialDesign();
};

enhanceRankingEditorialDesign();
/* ==================================================
   Syrup Atelier Ranking Parts
   調合レシピカード・メニュー札・在庫棚・レシートを追加
================================================== */

const rankingAtelierData = {
  "lonely-hidden": {
    recipe: {
      theme: "さみしさを隠しがち",
      base: "恋愛のクセ",
    ...
    menuLabels: ["本日の静かな一滴", "秘密の余白ブレンド", "あとから効く甘さ"],
    receipt: {
      topFlavor: "アールグレイシロップ",
      hiddenTaste: "言葉にしない寂しさ",
      aftertaste: "静かな余韻",
      message:
        "このランキングは、寂しさを感じた時にどれだけ表に出さず、自分の中で処理しようとするかを基準にしています。上位ほど「平気」と言いながら、内側で静かに揺れやすいタイプです。"
    }
  ...
  "addictive": {
    ...
    menuLabels: ["濃厚余韻シロップ", "秘密の中毒ブレンド", "刺激強めの一滴"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "甘さだけでは終わらない濃さ",
      aftertaste: "忘れられない余韻",
      message:
        "このランキングは、離れたあとにも思い出されやすい余韻や、もっと知りたいと思わせる奥行きを基準にしています。上位ほど、甘さだけでは終わらない記憶の残し方をします。"
    }
  ...
  "sweet-clumsy": {
    ...
    menuLabels: ["弱さガード多め", "上品な遠慮仕立て", "自由な甘え下手"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "弱さを見せる前の警戒心",
      aftertaste: "深く信じたい気持ち",
      message:
        "このランキングは、本当は甘えたいのに遠慮したり、強がったり、自分の気持ちを後回しにしやすいかを基準にしています。上位ほど、甘えたい気持ちを見せるまでに時間がかかります。"
    }
  },
  ...
  "heavy-when-serious": {
    ...
    menuLabels: ["濃すぎる本命", "安心欲しがり", "照れ隠し高温"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "軽く扱われたくない本気",
      aftertaste: "深く残る愛情",
      message:
        "このランキングは、好きが本気になった時にどれだけ深く考え、相手の反応に敏感になりやすいかを基準にしています。上位ほど、軽い恋より濃い関係を求めます。"
    }
  ...
  "easy-to-read-love": {
    ...
    menuLabels: ["素直な甘さ", "自然な接近", "やさしさ漏れ"],
    receipt: {
      topFlavor: "ストロベリーシロップ",
      hiddenTaste: "隠しきれない反応",
      aftertaste: "甘い好きバレ",
      message:
        "このランキングは、好きな人への反応や態度の変化がどれだけ周りに伝わりやすいかを基準にしています。上位ほど、本人は隠しているつもりでも好意がにじみます。"
    }
  ...
  "love-complicated": {
    ...
    menuLabels: ["深読み沼", "照れ隠し事故", "本音の迷宮"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "本気だからこその警戒",
      aftertaste: "複雑な余韻",
      message:
        "このランキングは、好きなのに素直になれなかったり、相手の反応を深読みしやすいかを基準にしています。上位ほど、恋が進むほど頭の中が忙しくなりやすいタイプです。"
    }
  ...
  "chase-more-than-be-chased": {
    ...
    menuLabels: ["刺激ハンター", "軽やか追跡", "深掘り衝動"],
    receipt: {
      topFlavor: "シナモンシロップ",
      hiddenTaste: "追うほど上がる温度",
      aftertaste: "ピリッと残る恋",
      message:
        "このランキングは、自分から気になっていく恋や、簡単に手に入らない相手に惹かれやすいかを基準にしています。上位ほど、恋に少しスパイスがあるほど燃えやすいです。"
    }
  ...
  "after-breakup-memory": {
    ...
    menuLabels: ["あとから効く", "濃厚記憶", "未完の余白"],
    receipt: {
      topFlavor: "キャラメルシロップ",
      hiddenTaste: "失って気づく優しさ",
      aftertaste: "日常に残る余韻",
      message:
        "このランキングは、離れたあとにどれだけ存在の大きさを思い出されやすいかを基準にしています。上位ほど、別れたあと日常の中でふと蘇るタイプです。"
    }
  ...
  "too-loyal": {
    ...
    menuLabels: ["深愛タイプ", "日常ごと愛す", "静かな一途"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "軽く切り替えられない情",
      aftertaste: "長く残る本気",
      message:
        "このランキングは、好きになった相手を長く大切にする力や、簡単に気持ちが薄れない情の深さを基準にしています。上位ほど、軽く切り替えるより長く想いを持ち続けます。"
    }
  ...
  "lover-only-gap": {
    ...
    menuLabels: ["濃厚ギャップ", "上品な甘さ", "特別枠"],
    receipt: {
      topFlavor: "ダークチョコシロップ",
      hiddenTaste: "恋人限定のやわらかさ",
      aftertaste: "特別な甘さ",
      message:
        "このランキングは、普段の印象と恋人にだけ見せる態度の差を基準にしています。上位ほど、心を許した相手にだけ見せる甘さや弱さが強く出ます。"
    }
...
function getCurrentAtelierRanking() {
  const title = document.querySelector(".ranking-title-area h3")?.textContent?.trim();
  if (!title || typeof flavorRankings === "undefined") return null;
...
function buildRecipeMeter(label, value) {
  return `
    <div class="recipe-meter">
      <span class="recipe-meter-label">${label}</span>
      <div class="recipe-meter-track">
        <div class="recipe-meter-fill" style="--meter-value:${value}%;"></div>
      </div>
      <span class="recipe-meter-value">${value}%</span>
    </div>
  `;
}

function buildRankingRecipeCard(ranking, data) {
  const recipe = data.recipe;

  return `
    <div class="ranking-recipe-card">
      <div class="recipe-head">
        <h4 class="recipe-title">調合レシピカード</h4>
        <span class="recipe-stamp">TASTING NOTE</span>
      </div>
      ...
      <div class="recipe-grid">
        <div class="recipe-info-list">
          <div class="recipe-info-row">
            <span>Theme</span>
            <strong>${recipe.theme}</strong>
          </div>
          <div class="recipe-info-row">
            <span>Base</span>
            <strong>${recipe.base}</strong>
          </div>
          <div class="recipe-info-row">
            <span>Taste</span>
            <strong>${recipe.taste}</strong>
          </div>
          <div class="recipe-info-row">
            <span>Spice</span>
            <strong>${recipe.spice}</strong>
          </div>
        </div>

        <div class="recipe-meter-list">
          ${Object.entries(recipe.meters)
            .map(([label, value]) => buildRecipeMeter(label, value))
            .join("")}
        </div>
      </div>
    </div>
...
function buildRankingReceipt(ranking, data) {
  return `
    <div class="ranking-receipt">
      <div class="receipt-title">
        <strong>RECEIPT</strong>
        <span>THANK YOU FOR TASTING</span>
      </div>

      <div class="receipt-lines">
        <div class="receipt-line">
          <span>Ranking Theme</span>
          <strong>${ranking.title}</strong>
        </div>
        <div class="receipt-line">
          <span>Top Flavor</span>
          <strong>${data.receipt.topFlavor}</strong>
        </div>
        <div class="receipt-line">
          <span>Hidden Taste</span>
          <strong>${data.receipt.hiddenTaste}</strong>
        </div>
        <div class="receipt-line">
          <span>Aftertaste</span>
          <strong>${data.receipt.aftertaste}</strong>
        </div>
      </div>

      <p class="receipt-message">${data.receipt.message}</p>
      <p class="receipt-thanks">Syrup Atelier / Love Type Lab</p>
    </div>
  `;
}

function enhanceSyrupAtelierRankingDesign() {
  const rankingBoard = document.getElementById("rankingBoard");
  if (!rankingBoard) return;

  const ranking = getCurrentAtelierRanking();
  if (!ranking) return;

  const data = rankingAtelierData[ranking.id];
  if (!data) return;

  const titleArea = rankingBoard.querySelector(".ranking-title-area");
  const top3 = rankingBoard.querySelector(".ranking-top3");
  const grid = rankingBoard.querySelector(".ranking-grid");

  if (titleArea && !rankingBoard.querySelector(".ranking-recipe-card")) {
    titleArea.insertAdjacentHTML("afterend", buildRankingRecipeCard(ranking, data));
  }

  if (top3) {
    top3.querySelectorAll(".ranking-hero-card").forEach((card, index) => {
      if (card.querySelector(".ranking-menu-label")) return;

      const label = document.createElement("div");
      label.className = "ranking-menu-label";
      label.textContent = data.menuLabels[index] || `おすすめ No.${index + 1}`;
      card.prepend(label);
    });
  }

  if (grid && !rankingBoard.querySelector(".ranking-stock-shelf-label")) {
    grid.insertAdjacentHTML(
      "beforebegin",
      `<div class="ranking-stock-shelf-label">Flavor Stock Shelf</div>`
    );
  }

  if (!rankingBoard.querySelector(".ranking-receipt")) {
    rankingBoard.insertAdjacentHTML("beforeend", buildRankingReceipt(ranking, data));
  }
}

const previousRenderRankingBoardForAtelier = renderRankingBoard;

renderRankingBoard = function (rankingId) {
  previousRenderRankingBoardForAtelier(rankingId);
  enhanceSyrupAtelierRankingDesign();
};
